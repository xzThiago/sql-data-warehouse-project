...
aprofundada utilizando técnicas de análise avançada em SQL. 
A análise busca identificar padrões de comportamento, desempenho e tendências ao longo do tempo.

Durante o projeto são aplicadas diferentes abordagens analíticas, 
incluindo análise de tendências temporais, métricas cumulativas, 
avaliação de desempenho, análise proporcional (parte-todo) e segmentação de dados.

Além disso, são construídos relatórios analíticos focados em clientes e produtos, 
consolidando métricas relevantes e KPIs importantes para apoiar a tomada de decisões baseadas em dados.

Importante:
No final de cada script é possível visualizar o resultado da consulta.
*/
